...
namespace UpkManager.Models.UpkFile.Engine.Mesh
{
    ...
    {
        // ─── Per-instance placement (UE3 SMC tagged properties) ──────────
        // Translation / Rotation / Scale / Scale3D are written by the editor
        // into the property stream when an SMC is placed in a level with a
        // non-identity transform. UStaticMeshActor's RootComponent.* values
        // surface here. Reading them is what unblocks per-instance scale and
        // makes 3D-viewer placement match in-game.
        ...
        [PropertyField] public FObject         StaticMesh { get; set; } // UStaticMesh
        [PropertyField] public UArray<FObject> Materials  { get; set; } // UMaterialInterface[]

        // ─── Lightmap binding (per-LOD) ──────────────────────────────────
        // LODData bulk-serializes after the property stream's None terminator.
        // The element reader (FStaticMeshComponentLODInfo.ReadData) is a stub
        // that consumes 0 bytes — calling it would mis-align the buffer and
        // crash any further reads on this object. Until the real FLightMap1D/
        // FLightMap2D licensee-specific binary decode is implemented (4G),
        // LODData is intentionally NOT parsed. Tagged properties parsed by
        // base.ReadBuffer (Translation/Rotation/Scale/StaticMesh/Materials)
        // are sufficient for per-instance placement.
        [StructField("StaticMeshComponentLODInfo")]
        public UArray<FStaticMeshComponentLODInfo> LODData { get; set; }

        // Cooked Marvel-Heroes SMC binary layout (verified via smcprobe on
        // HighTown_Terminal_X0_Y0_A.upk's `sma_black_floor_smc_77`):
        //   0x00..0x03: NetIndex (int32, usually 0)
        //   0x04..0x07: 0xFFFFFFFF — cooked-content marker (UE3 strips
        //               TemplateOwnerClass + TemplateName but leaves a
        //               placeholder int32, value always -1 in this sample)
        //   0x08+    :  FPropertyTag stream (first tag observed: "StaticMesh"
        //               ObjectProperty size=4 arrayIdx=0 + obj ref -112).
        //
        // We bypass UComponent.ReadBuffer (which would consume 12 bytes for
        // TemplateOwnerClass + TemplateName as if uncooked), and instead do
        // NetIndex + skip 4 + properties.
        public override void ReadBuffer(UBuffer buffer)
        {
            // NetIndex (private setter on UObject — set via reflection).
            int net = buffer.Reader.ReadInt32();
            typeof(UObject).GetProperty("NetIndex")?.SetValue(this, net);

            // Cooked marker (4 bytes). We don't interpret it, just consume.
            buffer.Reader.Skip(4);

            // Property loop. UObject.ReadProperties is private — invoke via
            // reflection so we don't reshape UObject's public surface.
            if (!buffer.IsAbstractClass)
            {
                var rp = typeof(UObject).GetMethod("ReadProperties",
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                rp?.Invoke(this, new object[] { buffer });
                var sp = typeof(UObject).GetMethod("SetProperties",
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
                sp?.Invoke(this, null);
            }
        }
