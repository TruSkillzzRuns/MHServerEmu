...
namespace UpkManager.Models.UpkFile.Types
{
    public class UBuffer(ByteArrayReader reader, UnrealHeader header)
    {
        ...
        // Like ReadBulkData() but also exposes the chunk metadata (flags +
        // offset into the .tfc file + sizes) so callers can stream the data
        // from a TextureFileCache when StoreInSeparateFile is set and Data
        // is otherwise empty.
        public byte[] ReadBulkDataExposed(out UnrealCompressedChunkBulkData chunk)
        {
            var bulkData = new UnrealCompressedChunkBulkData();
            bulkData.ReadCompressedChunk(Reader);
            var reader = Task.Run(() => bulkData.DecompressChunk(0)).Result;
